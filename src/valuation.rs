...
impl RationalAgent {
    ...
    // TODO: move this to the Stock impl.
    ...
    fn is_producible(&self, good: &Good) -> bool {
        ...
        // Capital good is not producible unless there is enough stock to finish production.
    ...
    fn marginal_benefit_of_producing_capital_goods(&self, good: &Good) -> f32 {
        ...
        if good == &Good::Axe {
            // println!(
            //     "productivity_per_unit_time for Axe: {:?}",
            //     productivity_per_unit_time
            // );
        }
    ...
    fn marginal_unit_value_of_capital_good(&self, good: &Good) -> f32 {
        ...
        let result = Good::iter()
            // .inspect(|x| println!("before filter: {:?}", x))
            .filter(|g| g.is_downsteam_of(good))
            // .inspect(|x| println!("after filter: {:?}", x)) //TMP
            // NO! .filter(|g| self.is_producible(g))
            // NO! .inspect(|x| println!("after producibility filter: {:?}", x)) //TMP
        ...
        // tmp
        if good == &Good::Axe {
            // println!("marginal_unit_value_of_capital_good for Axe: {:?}", result);
        }
        result
    }
